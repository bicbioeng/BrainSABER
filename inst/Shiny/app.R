...
server <- function(input, output) {
  ...
  output$recom <- DT::renderDataTable({
    if(isTruthy(input$dataSetSelected) && isTruthy(values$distance)){
      ...
      m <- values$distanceSet$meta[order(values$distanceSet$meta$Tau, decreasing = T),]
      m <- cbind(1:nrow(m), m)
      colnames(m)[1] <- "Rank"

      dt <- DT::datatable(m, rownames = F, selection = "single")
      dt }
    }, server = F)
