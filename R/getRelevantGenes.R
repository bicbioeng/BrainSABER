...
#'
getRelevantGenes <- function(data, dataSetId = NULL, AIBSARNAid = c("gene_id", 
    "ensembl_gene_id", "gene_symbol", "entrez_id", "refseq_ids")) {
    ...
    relfd <- fData(AIBSARNA::AIBSARNA)[vInd,] # subset feature data
    ...
    relexprs <- exprs(AIBSARNA::AIBSARNA)[vInd,] # subset exprs
    ...
    relevantGenes <- ExpressionSet(assayData = relexprs,
        phenoData = phenoData(AIBSARNA::AIBSARNA), featureData = relfd)
