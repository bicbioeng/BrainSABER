...
#' the genes \code{data}, which may be a vector or a Biobase ExpressionSet,
...
#' of numerical gene expression values with names comparable to one columns
...
getRelevantGenes <- function(data, dataSetId = NULL,
  ...
  relfd[[AIBSARNAid]] <- factor(relfd[[AIBSARNAid]],
                                levels = unique(names(v)))
  relfd <- relfd[order(relfd[[AIBSARNAid]]),]
  relfd$row_num <- factor(relfd$row_num, levels = unique(relfd$row_num))
  # convert relexprs to data.frame and add a column to do factor
  # manipulations
